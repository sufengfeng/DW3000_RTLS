...
{
    uint32_t ipatovPower, ipatovAccumCount, dgc_decision;
    ipatovPower = dwt_read32bitoffsetreg(IP_DIAG_1_ID, 0);
    ipatovAccumCount = dwt_read32bitoffsetreg(IP_DIAG_12_ID, 0);
    dgc_decision = dwt_read32bitoffsetreg(DGC_CFG_DBG_ID, 0);
    dgc_decision = dgc_decision >> 28;
    dgc_decision = dgc_decision & 0x07;
    //è®¡ç®—æŽ¥æ”¶åŠŸçŽ‡
    uint16 C,N;
    ...
    float estRxPwr = 10.0*log10(((float)C*(float)twoPower17)/((float)N*(float)N))+(6*dgc_decision)-A;
...
void dwt_setpdoaoffset(int16_t pdoaOffset_deg)
{
    ...
    if(pdoaOffset_deg < 0)
    {
        tmp = (int)(-1*(pdoaOffset_deg)*M_PI)*(1<<11)/180;
    }
    else if(pdoaOffset_deg > 0)
    {
        tmp = (int)((360 - (pdoaOffset_deg))*M_PI)*(1<<11)/180;
    }
    else
    {
        tmp = (int)((0)*M_PI)*(1<<11)/180;
    }

    tmp &= CIA_ADJUST_PDOA_ADJ_OFFSET_BIT_MASK;
    dwt_modify16bitoffsetreg(CIA_ADJUST_ID, 0, (uint16_t)~CIA_ADJUST_PDOA_ADJ_OFFSET_BIT_MASK, tmp);//å¯„å­˜å™¨å†™å…¥ä¸ºå¼§åº¦åç§»å€¼ï¼ŒèŒƒå›´ 0~2Î 
}
